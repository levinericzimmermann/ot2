...
def post_process_cengkok_5(
    ...
    keyboard = time_brackets_to_post_process[0][1]
    right_hand, left_hand = keyboard
    right_hand[-1].pitch_or_pitches.extend(left_hand[-1].pitch_or_pitches)
    left_hand[-1].pitch_or_pitches = []
...
"""
    ...
    time_brackets_to_post_process[0].set_parameter(
        "volume", parameters.volumes.WesternVolume("mp")
    )
    ...
    tw.add_cent_deviation_to_sequential_event(time_brackets_to_post_process[0][0][0])
    # tw.add_cent_deviation_to_sequential_event(time_brackets_to_post_process[0][1][0])
    # tw.add_cent_deviation_to_sequential_event(time_brackets_to_post_process[0][2][0])
