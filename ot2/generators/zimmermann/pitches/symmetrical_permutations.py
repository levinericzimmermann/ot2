...
class SymmetricalPermutation(object):
    ...
    def __init__(self, k: int, r: typing.Sequence[int]):
        self._harmonies, self._roots = SymmetricalPermutation._make_harmonies_and_roots(
            k, r
        )
    ...
    def _make_connections(
        ...
        for harmony0, harmony1, connection_pitches in (
            ...
            connections.update(
                {
                    (harmony0, harmony1): connection_pitches
                    + (pitches.JustIntonationPitch("1/1"),)
                }
            )
            connections.update(
                {
                    (harmony1, harmony0): connection_pitches
                    + (pitches.JustIntonationPitch("1/1"),)
                }
            )
    ...
    @property
    def roots(self) -> typing.Dict[str, typing.Tuple[pitches.JustIntonationPitch, ...]]:
        return self._roots
